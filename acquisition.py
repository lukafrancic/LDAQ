...
from LDAQ.acquisition_base import BaseAcquisition
import LDAQ
...
def _profile_callback(data, size, user_data):
    global _profile_buffer
    if user_data == 1:
        # parametri za memmove: dst, src, count
        ct.memmove(_profile_buffer, data, size)
        # nastavljen event za thread
        _event.set()


_event = threading.Event()
_profile_buffer = (ct.c_ubyte*(640 * 16))()


class MELaserScanner(BaseAcquisition):
    """
    This is a class for acquiring data from Micro Epsilon laser profile sensor.
    ...
    def profile_setup(self):
        ...
        for name, data_type in self._default_channel_names.items():
            if name in self._channel_names_init:
                self._buffer_arrays[name] = np.empty(self.resolution, data_type[0])
    ...
    def data_generator(self):
        ...
        while True:
            # c55
            # self.scanner_buffer.event.wait()

            # LLT class, Profile buffer, Partial profile, 
            # Profile configuration of transmission, Scanner type, Profile stripe to be evaluated,
            # Convert x/z data to millimeters, Array for reflection width, Array for maximum intensity, Array for threshold,
            # pX Array for position values, pZ Array for distance values, pM0 Array for moment 0, 
            # pM1 Array for moment 1
            # fret = llt.convert_part_profile_2_values(
            #     self.hLLT, self.scanner_buffer.profile_buffer,
            #     ct.byref(self.partial_profile_struct), self.scanner_type, 0,
            #     1, self._pointers["W"], self._pointers["I"], self._pointers["T"], 
            #     self._pointers["X"], self._pointers["Z"], self._pointers["M0"],
            #     self._pointers["M1"]
            #     )
            fret = llt.convert_part_profile_2_values(
                self.hLLT, _profile_buffer,
                ct.byref(self.partial_profile_struct), self.scanner_type, 0,
                1, self._pointers["W"], self._pointers["I"], self._pointers["T"], 
                self._pointers["X"], self._pointers["Z"], self._pointers["M0"],
                self._pointers["M1"]
...
def _data_generator(hLLT, profile_struct, sc_type, pointer_dict):
...
if __name__ == "__main__":
    ...
    print("crash???")
    vis.add_lines((0, 0), "me", (0,1))

    vis.config_subplot((0,0), xlim=(-50,50), ylim=(0, 100))
